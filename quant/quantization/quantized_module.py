...
class QLinear_transfer(QuantizedOperator, nn.Linear):
    ...
    def forward(self, input, multiplier_vector):
        weight_transfer = self.weight * multiplier_vector # change @ 10.18
        return F.linear(input, self.weight_fake_quant(weight_transfer), self.bias)

def Quantizer_transfer(module, config):
    kwargs = get_module_args(module)
    qmodule = QLinear_transfer(**kwargs, w_qconfig=config)
    qmodule.weight.data = module.weight.data.clone()
    if getattr(module, 'bias', None) is not None:
        qmodule.bias.data = module.bias.data.clone()
    return qmodule

class QuantizedLayer_transfer(QuantizedModule):
    def __init__(self, module, activation, w_qconfig, a_qconfig, qoutput=True):
        super().__init__()
        self.qoutput = qoutput
        self.module = Quantizer_transfer(module, w_qconfig)
        self.activation = activation
        if qoutput:
            self.layer_post_act_fake_quantize = Quantizer(None, a_qconfig)

    def forward(self, x, multiplier_vector):
        x = self.module(x, multiplier_vector)
        if self.activation is not None:
            x = self.activation(x)
        if self.qoutput:
            x = self.layer_post_act_fake_quantize(x)
