...
import logging
from quant.solver.imagenet_utils import DataSaverHook, StopForwardException
from quant.quantization.quantized_module import QuantizedModule
from quant.quantization.fake_quant import LSQFakeQuantize, LSQPlusFakeQuantize, QuantizeBase,GELULSQFakeQuantize
...
def save_inp_oup_data(model, module, cali_data: list, store_inp=False, store_oup=False, bs: int = 32, keep_gpu: bool = True):
    ...
    cached = [[], []]
    with torch.no_grad():
        for i in range(int(cali_data[0].size(0) / bs)):
            try:
                #_ = model(cali_data[i * bs: (i + 1) * bs].to(device))
                _ = model.forward(cali_data[0][i * bs: (i + 1) * bs].to(device),cali_data[1][i * bs: (i + 1) * bs].to(device),cali_data[2][i * bs: (i + 1) * bs].to(device))
...
def reconstruction(model, fp_model, module, fp_module, cali_data, config):
    ...
    for name, layer in module.named_modules():
        ...
        if isinstance(layer, QuantizeBase) and 'post_act_fake_quantize' in name:
            ...
            if isinstance(layer, GELULSQFakeQuantize):
                a_para += [layer.scale]                
    if len(a_para) != 0:
